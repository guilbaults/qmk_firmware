...
bool matrix_scan_custom(matrix_row_t current_matrix[]) {
    ...
    uint8_t uart_data[MATRIX_ROWS * 2 + 1] = {0};

    //there are 2*MATRIX_ROWS bytes corresponding to 2*MATRIX_ROWS columns, and then an end byte
    for (uint8_t i = 0; i < MATRIX_ROWS*2+1; i++) {
    ...
    if (uart_data[MATRIX_ROWS*2] == 0xE0) {
